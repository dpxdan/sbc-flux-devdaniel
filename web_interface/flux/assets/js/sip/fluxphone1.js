...
var beep = (function() {
    var fluxClass = window.audioContext || window.AudioContext || window.AudioContext || window.webkitAudioContext
    var flux = new fluxClass();
...
function notifyMe(msg) {
    ...
    if (Notification.permission === "granted") {
            ...
            let notification = new Notification('Ramal Flux', {
                body: msg,
                icon: img
            });
...
function handleNotify(r) {
    ...
    if (gotmsg) {
        ...
        if (newMessages) {
            $("#checkvmailbtn").removeClass('').addClass('btn-warning');

        } else {
            $("#checkvmailbtn").removeClass('btn-warning').addClass('');
...
function init() {
    ...
    ua = new SIP.UA({
        ...
        uri: uri,
        password: password,
        userAgentString: 'FluxPhone/5.2.9',
        traceSip: true,
...
$(document).ready(function() {
    audioElement.setAttribute('src', '/application/modules/webrtc/prod/audio/mp3/ring.mp3');
});
