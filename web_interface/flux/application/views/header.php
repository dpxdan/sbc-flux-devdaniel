...
<head>
	...
	<title>
		...
		<?php } else { ?>
			<? start_block_marker('page-title') ?><? end_block_marker() ?> | Flux Telecom - Unindo pessoas e negócios
		<?php } ?>
	</title>
...
<nav class="navbar navbar-expand-lg navbar-light bg-light mainmenu px-lg-4">
	...
	?>
		<a class="navbar-brand p-lg-0 m-0" href="<?php echo base_url(); ?>user/user/">
			<img id="logo" class="img-fluid" alt="user_logo" src="<?php echo base_url(); ?>upload/<?= $user_logo['logo'] ?>">
	...
	<div class="navbar-collapse collapse pl-lg-2 pr-0" id="main_menu" style="">
		<ul class="navbar-nav col-lg-10 col-md-12 p-0 mr-auto d-flex">
			...
			?>
				...
				if (
					...
					in_array($check_menu_key, $menu_permission_info) ||
					in_array($check_menu_key, $allow_module)
				) {
						...
						<?php if ($sub_menu_key === 0) { ?>
							...
							foreach ($sub_menu_values as $sub_key => $sub_menu_lables) {
								...
								if (
									...
									if (
										(common_model::$global_config['system_config']['paypal_status'] == 1 && strtolower($sub_menu_lables["menu_label"]) == 'recharge') ||
										(common_model::$global_config['system_config']['enterprise'] == 0 && $sub_menu_lables["menu_label"] == 'Opensips devices') ||
										(common_model::$global_config['system_config']['enterprise'] == 0 && $sub_menu_lables["menu_label"] == 'Opensips') ||
										(common_model::$global_config['system_config']['opensips'] == 1 && $sub_menu_lables["menu_label"] == 'Dispatcher list') ||
										(common_model::$global_config['system_config']['opensips'] == 1 && $sub_menu_lables["menu_label"] == 'Opensips devices') ||
										(common_model::$global_config['system_config']['opensips'] == 1 && $sub_menu_lables["menu_label"] == 'Opensips') ||
										(common_model::$global_config['system_config']['enterprise'] == 1 && common_model::$global_config['system_config']['opensips'] == 0 && $sub_menu_lables["menu_label"] == 'SIP Devices') ||
										(($acc_info['type'] == '3' || $acc_info['type'] == '0') && $acc_info['allow_ip_management'] == '1' && strtolower($sub_menu_lables["menu_label"]) == 'ip settings')
									) {
										/* item filtrado — nao exibir */
									} else { ?>

										<?php if (($this->session->userdata('userlevel_logintype') == '2')) { ?>
											<?php if (isset($permissioninfo[$module_url[0]][$module_url[1]])) { ?>
							...
							<ul class="col-4 col-12 dropdown-menu">
								...
								foreach ($sub_menu_values as $sub_menu_lables) {
									...
									if (
										isset($permissioninfo[$module_url[0]][$module_url[1]]) ||
										$this->session->userdata('userlevel_logintype') != '0' ||
										in_array($module_url[1], $allow_menu_url)
									) {
		...
		<div class="col navbar-profile dropdown">
			...
			<span>
				...
				<label class="text-success profile_label m-0"><?php echo $balance_str; ?></label>
			</span>
			</a>
			...
			<ul class="dropdown-menu dropdown-menu-right animated faster fadeIn rounded-0 m-0 w-100" aria-labelledby="admin_menu">
				...
				<?php if ($this->session->userdata('userlevel_logintype') != '2' && $this->session->userdata('userlevel_logintype') != '-1') { ?>
					<li><a class="dropdown-item" href="<?php echo base_url(); ?>user/user_myprofile/"><i class="fa fa-user"></i> &nbsp;<?php echo gettext('My Profile'); ?></a></li>
				<?php } ?>

				<?php if ($this->session->userdata('userlevel_logintype') == '-1') { ?>
					<li><a class="dropdown-item" href="/addons/addons_list/plugins"><i class="fa fa-plus"></i> &nbsp;<?php echo gettext('Plugins'); ?></a></li>
				<?php } ?>
...
<?php if ($this->session->userdata('userlevel_logintype') == '-1') { ?>
	...
	<div class="btn-quick">
		<div href="#q_menu" id="q_toggle" class="btn rounded-circle p-btn-1" data-action="rotate">
			<i class="fa fa-plus fa-2x"></i>
		</div>

		<div id="q_menu">
			<ul class="d-flex flex-column-reverse">
				<li><a href="<?php echo base_url(); ?>accounts/customer_add/" rel="facebox"><?php echo gettext("Customers"); ?></a></li>
...
<!-- ===================== QUICK FORM (popup hidden) ===================== -->
<div id="info" style="display:none;">
	<section class="slice m-0">
		<div class="w-section inverse p-0">
			<div class="col-md-12 p-0">
				<h3 class="text-dark fw4 px-4 pt-4 rounded-top"><?php echo gettext("Quick From"); ?></h3>
			</div>
		</div>
	</section>
	<section class="slice m-0">
					...
					<div class="col-12 p-0">
						<ul class="card p-0">
							<div class="pb-4" id="floating-label">
								<h3 class="bg-secondary text-light p-3 rounded-top"><?php echo gettext("General Details"); ?></h3>
								<li class="col-md-12 form-group">
									<label class="p-0 control-label"><?php echo gettext("Account"); ?> <span style="color:black;"> *</span></label>
								...
								<li class="col-md-12 form-group">
									<label class="p-0 control-label"><?php echo gettext("Amount"); ?> <span style="color:black;"> *</span></label>
									<input name="count" value="" size="20" class="col-md-12 form-control form-control-lg" type="text">
								</li>
							</div>
						</ul>
