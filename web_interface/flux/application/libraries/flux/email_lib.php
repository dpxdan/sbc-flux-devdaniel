...
class email_lib
{
	...
	function get_email_settings($resellerID = 1, $details = array())
	{
		...
		$this->smtp_status = isset($query[0]->smtp_status) && $query[0]->smtp_status != 1
			? $query[0]->smtp_status
			: $smtp_status;
	...
	function build_template($template_type, $detail, $detail_type = '')
	{
		...
		if (
			...
			is_array($templateinfo) &&
			isset($templateinfo['first_name']) &&
			$templateinfo['first_name'] != ''
		) {
	...
	function mail_history($attachment)
	{
		$send_mail_details = array(
			'from'       => $this->from,
			'to'         => $this->to,
			'cc'         => isset($this->cc) && $this->cc != '' ? $this->cc : null,
	...
	{
		$email_history_id = $this->send_notifications_email(
			$template_type,
			$details,
			$detail_type,
			$attachment,
		...
		if ($email_history_id !== false && !empty($email_history_id)) {
			$this->update_mail_history($email_history_id, 1, '', '', 'email');
		} elseif (!empty($details['history_id'])) {
			$this->update_mail_history($details['history_id'], 2, '', '', 'email');
		}

		$this->send_notifications_sms($template_type, $details, $detail_type, '', $resend, $mass_mail, $brodcast, '');
	}
	...
	{
		$history_id = '';
		$resellerID = array_key_exists('reseller_id', $details) ? (int) $details['reseller_id'] : 0;

		if (array_key_exists('history_id', $details)) {
			$history_id = $details['history_id'];
		}

		$this->get_email_settings($resellerID, $details);
	...
	function send_sms()
	{
		...
		if (!$this->sms_notications) {
			...
			$url = "https://rest.nexmo.com/sms/json?api_key=" . $this->sms_api_key .
				"&api_secret=" . $this->sms_secret_key .
				"&from=NEXMO&to=" . $country_code . "" . $contact_no .
				"&text=" . $this->sms_content . "";
	...
	function get_account_cc_emails($accountid, $primary_email = '')
	{
		...
		if (!empty($parent) && isset($parent['parent_id']) && (int) $parent['parent_id'] > 0) {
		...
		$this->CI->db->where_in('accountid', $account_ids);
