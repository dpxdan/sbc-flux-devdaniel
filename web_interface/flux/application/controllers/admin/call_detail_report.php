...
class Call_detail_report extends Account
{
    ...
    $this->db->where($object_where_params_date);
		...
		$this->db->limit($no_of_records, $start);        $this->db->select('callstart,sip_user,callerid,call_direction,callednum,notes,billseconds,disposition,debit,is_recording,country_id,pattern,cost,accountid,reseller_id,pricelist_id,calltype,trunk_id');
    ...
    $this->db->where($object_where_params_date);
		...
		$this->db->order_by("callstart", "desc");
		$this->db->limit($no_of_records, $start);  $this->db->select('calltype,callstart,sip_user,call_direction,country_id,callerid,callednum,pattern,notes,billseconds,provider_call_cost,disposition,provider_id,cost');
    $result = $this->db->get('cdrs');
    $count = $result -> num_rows();
    $cdrs_info = $result->result_array();
		foreach ($cdrs_info as $key => $cdrs_value) { 
