	...
	function _customer_list() {
		if (!isset($this->postdata['end_limit']) || !isset($this->postdata['start_limit'])) {
			...
			$query = $this->db
				...
				->order_by('id', 'desc')
				->select('id,id as accountid, number,first_name,timezone_id,last_name,company_name,email,reseller_id,telephone_1,postal_code,province,city,country_id,pricelist_id,posttoexternal,currency_id,balance,credit_limit,first_used,expiry,localization_id,creation,status,permission_id,tax_number')
				->where('deleted', '0')
				->get('accounts');
