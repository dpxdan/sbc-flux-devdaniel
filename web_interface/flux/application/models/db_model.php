...
//		$drp_list [0] = gettext("--Select--");
	...
	function build_concat_carrier_eot_select_dropdown($select, $table, $id_where = '', $id_value = '') {
		$select_params = explode ( ',', $select );
		if (isset ( $select_params [3] )) {
			$cnt_str = " $select_params[1],' ',$select_params[2],' ','(',$select_params[3],')' ";
		} else {
		...
		foreach ( $drp_array as $drp_value ) {
			if($select_params[1] == 'carrier_name'){
				$company_info = (array)$this->db->get_where($table,array("carrier_id"=>$drp_value->carrier_id))->first_row();
				if (!empty($company_info['carrier_rn1'])) {
					$drp_value->carrier_name = $company_info['carrier_name'] . '('.$company_info['carrier_eot'].' )';
				}else{
					$drp_value->carrier_name = $company_info['carrier_name'] . ' ' .'('.$company_info['accountid'] .')';
				}
			}else{
	...
	function build_concat_eot_select_dropdown($select, $table, $id_where = '', $id_value = '') {
    $select_params = explode(',', $select);
    ...
    $raw_key_alias = '_raw_key';
    $select = $select_params[0]
        . ", $select_params[1] as {$raw_key_alias}"
        . ", concat($cnt_str) as $select_params[1]";
...
//    $drp_list[0] = gettext("--Select--");
    ...
    foreach ($drp_array as $drp_value) {
        $key = $drp_value->{$raw_key_alias};
        $drp_list[$key] = $drp_value->{$select_params[1]};
    }
