...
class Invoice_model extends CI_Model
{
    ...
    public function check_existing_invoice($accountid, $from_date, $to_date)
    {
        $sql = "
            SELECT id
            FROM invoices
            WHERE accountid = ?
              AND is_deleted = 0
              AND DATE_FORMAT(from_date, '%Y-%m') = DATE_FORMAT(?, '%Y-%m')
              AND DATE_FORMAT(to_date, '%Y-%m')   = DATE_FORMAT(?, '%Y-%m')
            LIMIT 1
        ";
    
        $query = $this->db->query($sql, array($accountid, $from_date, $to_date));
        $row = $query->row_array();
        return isset($row['id']) ? intval($row['id']) : null;
    }

    
    public function generate_invoice_api_period($accountid, $from_date, $to_date)
    {
    $this->StartDate = date("Y-m-d 00:00:00", strtotime($from_date));
    $this->EndDate   = date("Y-m-d 00:00:00", strtotime($to_date));
    $this->CurrentDate = gmdate("Y-m-d H:i:s");

    $existing = $this->check_existing_invoice($accountid, $this->StartDate, $this->EndDate);
    if ($existing) {
        return $existing;
    }
    ...
    $accountinfo = $accountdata->row_array();
...
/*    if ($invoiceid > 0) {
    ...
    public function get_active_plan($accountid, $from_date)
    {
        ...
        $sql = "
            ...
            WHERE accountid = ?
              AND is_terminated = 0
              AND DATE_FORMAT(billing_date, '%Y-%m') = ?
            LIMIT 1
        ";

        $query = $this->db->query($sql, array($accountid, $month));
        return $query->row_array();
    }

    public function get_total_seconds($accountid, $from_date, $to_date)
    {
        $sql = "
            SELECT SUM(billseconds) AS total_seconds
            FROM cdrs
            WHERE accountid = ?
              AND callstart BETWEEN ? AND ?
        ";

        $query = $this->db->query($sql, array($accountid, $from_date, $to_date));
        $row = $query->row_array();
        return isset($row['total_seconds']) ? intval($row['total_seconds']) : 0;
    }

    public function get_exceeding_cost($accountid, $from_date, $to_date)
    {
        $sql = "
            SELECT SUM(debit) AS total_cost
            FROM cdrs
            WHERE accountid = ?
              AND callstart BETWEEN ? AND ?
              AND package_id = 0
        ...
        $query = $this->db->query($sql, array($accountid, $from_date, $to_date));
        $row = $query->row_array();
        return isset($row['total_cost']) ? floatval($row['total_cost']) : 0.0;
    }
        
    public function get_active_plans($accountid, $from_date)
    {
        $month = date("Y-m", strtotime($from_date));
