...
class Ratedeck extends MX_Controller
{
    ...
    {
        parent::__construct();
        $this->load->library('flux/form', 'ratedeck_form');
    ...
    function ratedeck_delete($id)
    {
        ...
        $this->session->set_flashdata('flux_notification', gettext('Ratedeck removed successfully!'));
        redirect(base_url() . 'ratedeck/ratedeck_list/');
    }
        ...
        if (isset($_FILES['ratedeckimport']['name']) && $_FILES['ratedeckimport']['name'] != "") {
            ...
            if ($ext == "csv" && $_FILES["ratedeckimport"]['size'] > 0) {
                ...
                if ($error == 0) {
                    ...
                    $full_path = $this->config->item('rates-file-path');
                    $actual_file_name = "FluxSBC-ratedeck-" . date("Y-m-d H:i:s") . "." . $ext;
    ...
    function ratedeck_import_file($check_header = false)
    {
        ...
        if ($count > 0) {
            ...
            $this->session->set_flashdata('flux_errormsg', 'Total ' . count($new_final_arr) . 'Ratedeck Imported Successfully!');
            redirect(base_url() . "ratedeck/ratedeck_list/");
        }
    }
