...
class Addons extends MX_Controller
{
     ...
     */
    function recursiveFiles($dir, $flux_path = '', $remove_path = '')
    {
        $files_array = array();
        $tree = glob(rtrim($dir, '/') . '/*');
        if (is_array($tree)) {
            ...
            foreach ($tree as $file) {
                if (is_dir($file)) {
                    // ~ $file1="";
                    $file1 = str_replace($remove_path, $flux_path, $file);
                    if (! is_dir($file1)) {
                        $files_array[] = $file1;
                    }
                    $files_array[] = $this->recursiveFiles($file, $flux_path, $remove_path);
                } elseif (is_file($file)) {
                    $files_array[] = $file;
                }
            }
        ...
        if ($module != "" && $action != "") {
            ...
            if ($action == 'install' or $action == "update") {
                ...
                if (is_dir($addons_path . $type . "/" . $module . "/web_interface/flux")) {
                    ...
                    // ~ echo $addons_path.$type."/".$module."/web_interface/flux"; exit;
                    // Copy files to web folder
                    echo "Copying file to web directories...<br/>";
                    $remove_path = $addons_path . $type . "/" . $module . "/web_interface/flux/";
                    $add_path = $flux_path;
                    $path = $addons_path . $type . "/" . $module . "/web_interface/flux/";
                    $scans = scandir($path);
                    $result_array = $this->recursiveFiles($path, $flux_path, $remove_path);
                    ...
                    $last_line = system("/bin/cp -rf " . $addons_path . $type . "/" . $module . "/web_interface/flux/. " . $flux_path, $retval);
                    if ($retval == '1') {
                ...
                // if (is_dir ($addons_path.$type."/".$module."/freeswitch/scripts/flux/lib/addons/"))
                if (is_dir($addons_path . $type . "/" . $module . "/freeswitch/scripts/flux/lib/")) {
                    ...
                    // $path = $addons_path.$type."/".$module."/freeswitch/scripts/flux/lib/addons/";
                    $path = $addons_path . $type . "/" . $module . "/freeswitch/scripts/flux/";
                    ...
                    system("/bin/mkdir -p " . $fs_usr_path . "scripts/flux/lib/");
                    $last_line = system("/bin/cp -rf " . $addons_path . $type . "/" . $module . "/freeswitch/scripts/flux/lib/. " . $fs_usr_path . "scripts/flux/lib/.", $retval);
                    ...
                    if ($retval == '1') {
                        ...
                        echo "<span style='color:red;'>Failed!! Unable to copy files to /usr/share/freeswitch/scripts/flux/lib/ folder!!!</span><br/><br/>";
                        echo "<a href='" . base_url() . "addons/addons_list/" . $type . "'>Back</a>";
