...
class Systems extends MX_Controller
{
    ...
    function configuration($group_title = '')
    {
        ...
        if (! empty($add_array)) {
            ...
            $this->session->set_flashdata('flux_errormsg', gettext(sprintf('%s Settings updated sucessfully!', ucfirst($group_title))));
    ...
    function timezone_remove($id)
    {
        ...
        $this->session->set_flashdata('flux_notification', $timezonename.' '.gettext('Timezone Removed Successfully!'));
    ...
    function resellertemplate_save($data, $resellerid)
    {
        ...
        if ($count > 0) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $this->session->set_flashdata('flux_errormsg', gettext('Template Updated Successfully!'));
    ...
    function country_remove($id)
    {
        ...
        $this->session->set_flashdata('flux_notification', $country.' '.gettext('Country removed successfully!'));
    ...
    function currency_remove($id)
    {
        ...
        $this->session->set_flashdata('flux_notification', $currencyname.' '.gettext('Currency Removed Successfully!'));
    ...
    function database_restore_one($id = '')
    {
        ...
        if ($result->num_rows() > 0) {
            ...
            if (file_exists($path)) {
                ...
                exec($run_restore);
                $this->session->set_flashdata('flux_errormsg', gettext('Database Restore successfully.'));
        ...
        if ($_POST['fname'] !='' && $_FILES['userfile']['name'] != '' && $_FILES['userfile']['type'] == "application/gzip") {
            ...
            if (file_exists($filename)) {
                ...
                if ($db_file[1] == 'csv' || $db_file[1] == 'tar' || $db_file[1] == 'sql') {
                    ...
                    $this->session->set_flashdata('flux_errormsg', "The file " . basename($_FILES['userfile']['name']) . " has been uploaded");
                    redirect(base_url() . "systems/database_restore/");
                } else {
                    $this->session->set_flashdata('flux_notification', gettext("There is a some issue or invalid file format."));
                    redirect(base_url() . "systems/database_restore/");
                }
            }
        }else{
    ...
    function languages_remove($id)
    {
        ...
        $this->session->set_flashdata('flux_notification', $languagename.' '.gettext('Languages removed successfully!'));
    ...
    function translation_remove($id)
    {
        ...
        $this->session->set_flashdata('flux_notification', gettext(sprintf('%s Translation removed successfully!', $translation_info)));
    ...
    function translation_save()
    {
        ...
        if (isset($add_array['id']) && $add_array['id'] != '') {
            if ($this->form_validation->run() == FALSE) {
                ...
                $this->session->set_flashdata('flux_errormsg', ucfirst($add_array["module_name"]) .' '. gettext('Translation updated successfully!'));
                redirect(base_url() . 'systems/translation_list/');
            }
        } else {
            if ($this->form_validation->run() == FALSE) {
                ...
                $this->session->set_flashdata('flux_errormsg', ucfirst($add_array["module_name"]) .' '. gettext('Translation added successfully!'));
                redirect(base_url() . 'systems/translation_list/');
            }
        }
    }
    ...
    function payment_methods()
    {
        ...
        if (! empty($add_array)) {
            ...
            $this->session->set_flashdata('flux_errormsg', ucfirst($group_title).' '.gettext('Settings updated sucessfully!'));
