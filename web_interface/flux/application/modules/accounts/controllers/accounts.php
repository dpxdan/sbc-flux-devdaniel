...
class Accounts extends MX_Controller {
	...
	function __construct() {
		...
		$this->load->library('flux/form', 'accounts_form');
	...
	function customer_edit($edit_id = '') {
		...
		if ((isset($access_edit)) && ($access_edit['deleted'] == 0)) {
			...
			if ($account->num_rows() > 0) {
				...
				//                $data['domain_id'] = $account_data['domain_id'];
				//                $domain_name = strtolower($this->common->get_entity_type('', '', $account_data['domain_id']));
				...
				if (isset($domains_data) && $domains_data->num_rows() > 0) {
					...
					$account_data["domain_id"] = explode(",", $domains_data[0]['domains_id']);
				}
	...
	function customer_save($add_array = false) {
		if ($_SERVER['REQUEST_METHOD'] === 'POST') {
			...
			//            $data['domain_id'] = $add_array['domain_id'];
			$data['entity_name'] = $entity_name;
			$data['callingcard'] = Common_model::$global_config['system_config']['pinlength'];
			...
			if ($add_array['id'] != '') {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$this->session->set_flashdata('flux_errormsg', gettext(ucfirst($entity_name)).' '.gettext('Updated successfully!'));
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$this->session->set_flashdata('flux_errormsg', gettext(ucfirst($entity_name)).' '.gettext('Added Successfully!'));
	...
	function customer_speeddial_save($number, $accountid, $speed_num) {
		if (is_numeric($number)) {
			...
			if ($account->num_rows() == 0) {
				...
				$this->session->set_flashdata('flux_errormsg', $number.' '.gettext('Speed Dial Number Added Successfully'));
			} else {
				$updateinfo = array(
					'number' => $number,
				);
				if (is_numeric($updateinfo['number'])) {
					...
					$this->session->set_flashdata('flux_errormsg', $updateinfo['number'].' '.gettext('Speed Dial Number Updated Successfully'));
				} else {
					$this->session->set_flashdata('flux_notification', gettext('Please insert only numeric value!'));
				}
			}
		} else {
			$this->session->set_flashdata('flux_notification', gettext('Please insert only numeric value!'));
	...
	function customer_speeddial_remove($accountid, $speed_num) {
		...
		if (!empty($number['number'])) {
			...
			$this->db->where('accountid', $accountid);
			$result = $this->db->update('speed_dial', $updateinfo);
			$this->session->set_flashdata('flux_notification', $number['number'].' '.gettext('Speed Dial Number Removed Successfully'));
		} else {
			$this->session->set_flashdata('flux_notification', gettext('Speed Dial Number is Empty'));
		}
	}
	...
	function customer_ipmap_action($action, $accountid, $accounttype, $ipmapid = "") {
		...
		if ($action == "add" && !empty($add_array)) {
			if ($add_array['ip'] != "") {
				...
				if ($getdata > 0) {
					$this->session->set_flashdata('flux_notification', gettext('IP already exist in system.'));
				} else {
					...
					if ($ip_flag) {
						...
						$this->session->set_userdata('flux_notification', $response);
					}
					$this->session->set_flashdata('flux_errormsg', gettext('IP added sucessfully.'));
				}
			}
		}
		if ($action == "delete") {
		...
		if ($action == "add") {
			...
			if ($count == 0) {
				if ($ani['number'] != "") {
					...
					$this->session->set_flashdata('flux_errormsg', gettext('Caller ID added successfully!'));
				} else {
					$this->session->set_flashdata('flux_notification', gettext('Please Enter Caller ID value.'));
				}
			} else {
				$this->session->set_flashdata('flux_notification', gettext('Caller ID already Exists.'));
			}
		}
		if ($action == "delete") {
			$this->session->set_flashdata('flux_notification', gettext('Caller ID removed sucessfully!'));
	...
	function customer_fssipdevices_action($action, $id, $accountid) {
		...
		if ($action == "delete") {
			...
			$this->session->set_flashdata('flux_notification', gettext('Sip Device removed successfully!'));
			redirect(base_url()."accounts/".$entity_type."_sipdevices/$accountid/");
		}
		if ($action == "edit") {
			$this->freeswitch->customer_fssipdevices_edit($id, $accountid);
			$this->session->set_flashdata('flux_errormsg', gettext('Sip updated successfully!'));
		}
	}
	...
	function customer_dids_action($action, $accountid, $accounttype, $did_id = "") {
		...
		if ($did_id != "") {
			...
			if ($action == "add") {
				...
				$this->session->set_flashdata($flux_flash_message_type, $did_result[1]);
				/*else {

			$flux_flash_message_type = ($did_result[0] == "INSUFFIECIENT_BALANCE") ? "flux_notification" : "flux_errormsg";
			$this->session->set_flashdata($flux_flash_message_type, $did_result[1]);
			 */
			}
			if ($action == "delete") {
				...
				$last_inserted_id                = $this->flux_sms->send_sms('email_remove_did', $account_arr, '');
	...
	function customer_delete_block_pattern($accountid, $patternid) {
		...
		if (!empty($entity_type)) {
			...
			$this->session->set_flashdata('flux_notification', gettext('Block Code Removed Sucessfully!'));
		} else {
			$this->session->set_flashdata('flux_notification', gettext('Permission Denied!'));
		}
		redirect(base_url().$url);
		...
		if (isset($add_array['id']) && !empty($add_array['id'])) {
			...
			if ($account_res->num_rows() > 0) {
				if ($this->form_validation->run() == FALSE) {
					...
					$this->session->set_flashdata('flux_errormsg', gettext('Alert threshold updated successfully!'));
		...
		if (!empty($post_array)) {
			if ($this->form_validation->run() == FALSE) {
				...
				if ($post_array['flag'] == '1') {
					...
					$this->session->set_flashdata('flux_errormsg', gettext('Account callerID updated successfully!'));
					redirect(base_url().'accounts/'.$entity_type.'_add_callerid/'.$id."/");
					exit();
				} else {
					$this->accounts_model->add_callerid($post_array);
					$entity_type = 'customer';
					$this->session->set_flashdata('flux_errormsg', gettext('Account callerID added successfully!'));
					redirect(base_url().'accounts/'.$entity_type.'_add_callerid/'.$id."/");
				}
			}
		}
		...
		if ($_SERVER['REQUEST_METHOD'] === 'POST') {
			...
			if ($add_array['id'] != '') {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$add_array['password'] = $this->common->encode($add_array['password']);
					$this->accounts_model->edit_account($add_array, $add_array['id']);
					$this->session->set_flashdata('flux_errormsg', gettext('Reseller updated successfully!'));
					redirect(base_url().'accounts/reseller_list/');
					exit();
				}
				$data["account_data"]["0"] = $add_array;
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$add_array['notification_email'] = (!empty($add_array['notification_email']))?$add_array['notification_email']:$add_array['email'];
					$last_id                         = $this->accounts_model->add_account($add_array);
					$this->session->set_flashdata('flux_errormsg', gettext('Reseller Added Successfully!'));
					redirect(base_url().'accounts/reseller_list/');
					exit();
				}
				$this->load->view('view_accounts_create', $data);
...
//		$entity_type         = strtolower($this->common->default_signup_login_type('', '', $type));
		...
		$data['back_flag']   = true;
		$data['form']        = $this->form->build_form($this->accounts_form->get_form_admin_fields($entity_type), '');
	...
	function customer_delete($id) {
		...
		if ($account_res->num_rows > 0) {
			...
			$this->session->set_flashdata('flux_notification', gettext('Customer removed successfully!'));
		} else {
			$this->session->set_flashdata('flux_notification', gettext('Permission Denied!'));
		}
		redirect(base_url().'accounts/customer_list/');
	...
	function reseller_delete($id) {
		...
		if ($account_res->num_rows > 0) {
			...
			$this->session->set_flashdata('flux_notification', gettext('Reseller removed successfully!'));
		} else {
			$this->session->set_flashdata('flux_notification', gettext('Permission Denied!'));
		}
		redirect(base_url().'accounts/reseller_list/');
	...
	function provider_delete($id) {
		...
		if ($account_res->num_rows > 0) {
			...
			$this->session->set_flashdata('flux_notification', gettext('Provider removed successfully!'));
		}
		redirect(base_url().'accounts/customer_list/');
	}
	...
	function admin_delete($id) {
		...
		$this->session->set_flashdata('flux_notification', gettext('Admin removed successfully!'));
		redirect(base_url().'accounts/admin_list/');
	}

	function subadmin_delete($id) {
		$this->accounts_model->remove_customer($id, 4);
		$this->session->set_flashdata('flux_notification', gettext('Sub admin removed successfully!'));
		redirect(base_url().'accounts/admin_list/');
	}
	...
	function reseller_did_action($action, $accountid, $accounttype, $did_id = "") {
		...
		if ($action == "delete") {
			...
			if ($pricing_res->num_rows() > 0) {
				...
				$this->session->set_flashdata('flux_notification', gettext('DID already removed before.'));
			}
			redirect(base_url()."accounts/".$accounttype."_edit/$accountid#did");
		}
	}
	...
	function customer_account_taxes($action = false, $id = false) {
		...
		if ($action == 'list') {
			...
			if (($this->input->post())) {
				...
				$this->session->set_flashdata('flux_errormsg', gettext('Account tax added successfully!'));
				redirect(base_url().'accounts/customer_list/');
			}
			$data['id']        = array();
			...
			$this->session->set_flashdata('flux_notification', gettext('Account tax removed successfully!'));
			redirect(base_url().'accounting/account_taxes/');
		}
	}

	function customer_account_domains($action = false, $id = false) {
		$data['username']   = $this->session->userdata('user_name');
		$data['page_title'] = gettext('Account Domains');
		...
		if ($action == 'list') {
			$this->load->view('view_account_domains_list', $data);
		} elseif ($action == 'add') {

			if (($this->input->post())) {
				$post_array = $this->input->post();
				$query      = $this->accounts_model->remove_all_account_domains($post_array['account_id']);
				...
				foreach ($post_array as $key => $value) {
					...
					if ($id[0] == 'domain') {
						$data = array(
							'accountid' => $post_array['account_id'],
							'domain_id' => $post_array[$key],
						);
						$this->accounts_model->add_account_domain($data);
					}
				}
				$this->session->set_flashdata('flux_errormsg', gettext('Account domain added successfully!'));
				redirect(base_url().'accounts/customer_list/');
			}
			$data['id']          = array();
			$data['domainsList'] = $this->common_model->get_list_domains();
			$this->load->view('view_accounting_domains_add', $data);
		} elseif ($action == 'edit') {
			$domain_id          = $this->accounts_model->get_accountdomain_by_id($id);
			$account_num        = $this->accounts_model->get_account_number($id);
			$data['accountnum'] = $account_num['number'];
			$data['account_id'] = $id;
			for ($i = 0; $i < count($domain_id); $i++) {
				$domain_id[] = $domain_id[$i]['domain_id'];
			}
			...
			if (($this->input->post())) {
				$post_array  = $this->input->post();
				$accountinfo = $this->accounts_model->get_account_by_number($post_array['account_id']);
				$query       = $this->accounts_model->remove_all_account_domain($post_array['account_id']);
				foreach ($post_array as $key => $value) {
					$id = explode("_", $key);
					if ($id[0] == 'tax') {
						$data = array(
			...
			$this->accounting_model->remove_account_domain($id);
			$this->session->set_flashdata('flux_notification', gettext('Account Domain removed successfully!'));
			redirect(base_url().'accounting/account_domains/');
		}
	}
	...
	function customer_product($edit_id) {
		...
		if ($account_res->num_rows > 0) {
			...
			$data['productslist'] = form_dropdown_all(array(
				...
				), $this->Flux_common->list_applyable_products($edit_id), '');
			$this->load->module('products/products');
	...
	function customer_products_action($action, $accountid, $accounttype, $product_id = "") {
		...
		if ($action == "delete") {
			...
			$this->session->set_flashdata('flux_notification', gettext('Product Removed Sucessfully.'));
		}
	}
