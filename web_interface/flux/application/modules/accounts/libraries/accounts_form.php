...
//		$parent_id  = ($parent_id > 0)?$parent_id:(($account_data['type'] == 0 || $account_data['type'] == 3)?$account_data['id']:0);
		...
		$block_type_status = array(
			gettext('Block Type Status'),
			'block_status',
			'SELECT',
			'',
			'',
	...
	function get_form_admin_fields($entity_type = '', $id = false) {
		...
		if ($id > 0) {
			...
			$token_api = $this->CI->common->generate_password();
			$token_api = array(
				gettext('Token API'),
				'INPUT',
				array(
...
//		$type          = $accountinfo['permission_id'];
	...
	function customer_batch_update_form() {
		...
		$block_type = array(
			gettext('Block Type Status'),
			array(
				'name'  => 'block_status[block_status]',
				'id'    => 'block_status',
				'class' => 'block_status',
			),
			'SELECT',
			...
			'name',
			'',
			'set_block_type_status',
			'',
			'',
			array(
				'name'  => 'block_status[operator]',
				'class' => 'update_drp',
			),
			'update_drp_type',
		);
