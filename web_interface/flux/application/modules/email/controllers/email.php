...
class Email extends MX_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->library('flux/form', 'email_form');
...
//        function send_notifications($template_type, $details, $detail_type = '', $attachment = '', $resend = 0, $mass_mail = 0, $brodcast = 0) {
    ...
    function email_delete($id)
    {
        ...
        $this->session->set_flashdata('flux_notification', gettext('Email removed successfully!'));
        redirect(base_url() . 'email/email_history_list/');
    }
    ...
    function email_send_multipal()
    {
        ...
        if ($add_array['email'] == '' || $add_array['subject'] == '' || $add_array['template'] == '') {
            $this->session->set_flashdata('flux_notification', gettext('Email address not found!'));
        ...
        exec($command);
        $this->session->set_flashdata('flux_errormsg', gettext('Email broad cast successfully!'));
        redirect(base_url() . 'email/email_history_list/');
    }
