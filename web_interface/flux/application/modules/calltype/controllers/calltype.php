...
class calltype extends MX_Controller
{
    ...
    function calltype_save()
    {
        ...
        if ($add_array['id'] != '') {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $this->session->set_flashdata('flux_errormsg', gettext('Calltype Added Successfully!'));
                echo json_encode(array(
    ...
    function calltype_delete($id)
    {
        ...
        $this->session->set_flashdata('flux_notification', gettext('calltype removed successfully!'));
        foreach ($accountinfo->result_array() as $key => $value) {

            // ------------------------------------------------------------------------------------------------------------
            $last_inserted_id = $this->flux_sms->send_sms('remove_calltype', $value, '');
            // ------------------------------------------------------------------------------------------------------------
