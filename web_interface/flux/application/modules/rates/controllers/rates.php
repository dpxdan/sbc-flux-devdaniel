...
class Rates extends MX_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->library('flux/form', 'rates_form');
                        ...
                        if (! in_array($mime_type, $acceptable_mime_types)) {
                            ...
                            if ($error == 0) {
                                ...
                                if (! empty($csv_data)) {
                                    ...
                                    $actual_file_name = "FluxSBC-TERMINATION-RATES-" . date("Y-m-d H:i:s") . "." . $ext;
    ...
    function termination_rate_rates_import($trunkID, $check_header = false)
    {
        ...
        if ($count > 0) {
            ...
            $this->session->set_flashdata('flux_errormsg', gettext('Total').' '. count($new_final_arr) .' '. gettext('Termination Rates Imported Successfully!'));
                ...
                if ($ext == "csv" && $_FILES['origination_rate_import']['size'] > 0) {
                    ...
                    if (! in_array($mime_type, $acceptable_mime_types)) {
                        ...
                        if ($error == 0) {
                            ...
                            if (! empty($csv_data)) {
                                $full_path = $this->config->item('rates-file-path');
                                $actual_file_name = "FluxSBC-ORIGIN-RATES-" . date("Y-m-d H:i:s") . "." . $ext;
                                if (move_uploaded_file($uploadedFile, $full_path . $actual_file_name)) {
                                    $flag = false;
    ...
    function origination_rate_import_file($pricelistID, $trunkid, $check_header = false)
    {
        ...
        if ($count > 0) {
            ...
            $this->session->set_flashdata('flux_errormsg', gettext('Total').' '.count($new_final_arr).' '.gettext('Origination Rate Imported Successfully!'));
            redirect(base_url() . "rates/origination_rates_list/");
        }
    }
    ...
    {
        $this->load->helper('download');
        //FLUXUPDATE-960 + 961 Start
        $full_path = "./assets/Rates_File/" . $file_name . ".csv";
        //FLUXUPDATE-960 + 961 End
        ob_clean();
    ...
    function termination_rate_rates_mapper_import()
    {
        ...
        if ($count > 0) {
            ...
            $this->session->set_flashdata('flux_errormsg', 'Total ' . count($new_final_arr) . ' Termination Rates Imported Successfully!');
