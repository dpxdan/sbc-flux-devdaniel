...
class Event_guard extends MX_Controller
{
    ...
    $msg = isset($result['ip'])
        ? $result['ip'] . ' ' . gettext('removed from block.')
    ...
    function event_guard_block($id = '')
    {
        ...
        if (!in_array($log['log_status'], array('unblocked', 'tracking'))) {
            $this->session->set_flashdata('flux_notification', gettext('IP is not unblocked.'));
            redirect(base_url() . 'event_guard/event_guard_list/');
        }
    ...
    function event_guard_log_save()
    {
        ...
        $safe_data = array(
            ...
            'failures'   => (int)  ($add_array['failures'] ?? 0),
    ...
    function event_guard_whitelist_delete_multiple()
    {
    ...
    private function _do_unblock($id)
    {
        $log_id = (int) $id;
        ...
        if (!in_array($log['log_status'], array('blocked', 'tracking'))) {
