...
class Event_guard_model extends CI_Model
{
    ...
    function get_event_guard_list($flag, $start = 0, $limit = 0)
    {
    ...
    function get_logs($filters = array(), $limit = 100, $start = 0)
    {
        $where = array();
    ...
    function count_logs($filters = array())
    {
        $where = array();
    ...
    function get_whitelist_by_id($id)
    {
    ...
    function get_whitelist_cidr($id)
    {
    ...
    function whitelist_cidr_exists($cidr, $exclude_id = 0)
    {
    ...
    function edit_whitelist($data, $id)
    {
        $data['created_at'] = gmdate('Y-m-d H:i:s');
        $this->db->where('id', (int) $id);
    ...
    function get_whitelist_list($flag, $start = 0, $limit = 0)
    {
    ...
    function delete_multiple_logs($ids)
{
    ...
    function get_log_by_uuid($log_uuid)
    {
    ...
    function set_unblocked($log_uuid)
    {
        $this->db->where('log_uuid', $log_uuid);
        $unblock_statuses = array('blocked', 'tracking');
        $this->db->where_in('log_status', $unblock_statuses);
        $this->db->update('event_guard_logs', array(
            'log_status' => 'unblocked',
            'log_date'   => gmdate('Y-m-d H:i:s'),
        ));
        return $this->db->affected_rows() > 0;
    }
    
    function set_blocked($log_uuid)
    {
        $this->db->where('log_uuid', $log_uuid);
    ...
    function delete_multiple_whitelist($ids)
    {
        if (empty($ids) || !is_array($ids)) {
            return false;
        }
    ...
    function get_recent_blocked_ips($limit = 5)
    {
        $query = $this->db_model->select(
