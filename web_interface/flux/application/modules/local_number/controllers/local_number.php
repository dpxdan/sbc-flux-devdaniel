...
class Local_number extends MX_Controller {
	function __construct() {
		...
		$this->load->library ( 'flux/form','local_number_form' );
					...
					if ($ext == "csv" && $_FILES ["localnumberimport"] ['size'] > 0) {
                        ...
                        if (! in_array($mime_type, $acceptable_mime_types)) {
							...
							if ($error == 0) {
								...
								$full_path = $this->config->item ( 'rates-file-path' );
								$actual_file_name = "FluxSBC-Local-Number-" . date ( "Y-m-d H:i:s" ) . "." . $ext;
	...
	function local_number_import_file($check_header = false) {
		...
		if ($count > 0) {
			...
			$this->session->set_flashdata ( 'flux_errormsg', 'Total ' . count ( $new_final_arr ) . ' Local Number Imported Successfully!' );
