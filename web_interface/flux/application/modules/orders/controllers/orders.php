...
class Orders extends MX_Controller
{
    ...
    function orders_save()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
			...
			$final_array['name'] = $ProductData['product_name'];
                    ...
                    $this->session->set_flashdata('flux_errormsg', gettext("Product assign successfully"));
                    redirect(base_url() . 'orders/orders_complete/' . $last_id . '');
                } else {
                    $this->session->set_flashdata('flux_notification', gettext($account_balance. '' .$total_amt));
    ...
    public function orders_complete($orderid)
    {
        ...
        if ($orderid != '') {
            ...
            if ($query->num_rows > 0) {
                ...
                $this->session->set_flashdata('flux_notification', gettext('Order Not found!'));
                redirect(base_url() . 'orders/orders_list/');
            }
        }
    }
    ...
    function orders_terminate($orderid)
    {
        ...
        $this->load->view("view_terminate_order", $data);
    }
