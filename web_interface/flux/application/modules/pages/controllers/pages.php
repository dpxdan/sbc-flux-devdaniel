	...
	{  
		...
		if (!$this->session->userdata('accountinfo'))
		{
			...
			if(!isset($productarr['error'])){
				...
				$this->session->set_flashdata ( 'flux_notification',  gettext('Something went worong !') );
		...
		if(!isset($productinfo['error'])){	
			...
			if($payment_method == "account_balance" && $productinfo['product_category'] != 3){  		
				...
				if($account_info ['type'] == 0 && (!$this->check_calls_running())){
					$this->session->set_flashdata('flux_notification', gettext('You can not purchase any new product when your calls are running!'));
				...
				if( $account_balance  >= $total_amt ){ 
					...
					if($order_id != ""){
						...
						$this->session->set_flashdata ( 'flux_errormsg',  gettext('Product Purchased successfully  !' ));
						redirect ( base_url () . 'pages/services/'.$productinfo['product_category'].'' );
					}
				}else{
					
					$this->session->set_flashdata ( 'flux_notification',  gettext('Insufficent Balance to purchase product  !' ));
					redirect ( base_url () . 'pages/services/'.$productinfo['product_category'].'' );
			        }
			}else{
    ...
    'verify_sign' => 'Aj8.yGaDEEvG5R4uDtSEL7b94tWUA0Yl8A2qA2R24.wefCGLe.z7sLgv'
		...
		if (count ( $_POST ) > 0) {
			...
			if(!empty($orderarr)){ 
				...
				{
					 $this->session->set_flashdata ( 'flux_errormsg',  gettext('Refill Done successfully!' ));
					...
					redirect ( base_url () . 'pages/services/'.$orderarr["product_category"].' ' );
					 ...
					 $this->session->set_flashdata ( 'flux_notification',  gettext('Something went worng!') );
					redirect ( base_url () . 'pages/services/'.$orderarr["product_category"].' ' );
