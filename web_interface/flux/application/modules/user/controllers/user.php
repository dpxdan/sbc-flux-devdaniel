...
class User extends MX_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->library("flux/form", 'user_form');
        ...
        if ($did_id != '') {
            ...
            if ($action == "add") {
                if ($accountinfo['type'] == 0 && (!$this->check_calls_running())) {
                    $this->session->set_flashdata('flux_notification', gettext('You can not purchase any new DID when your calls are running!'));
            ...
            $this->session->set_flashdata('flux_notification', gettext('DID not found.'));
            redirect(base_url() . "user/user_didlist/");
        }
    }
    ...
    function change_password()
    {
        ...
        if ($databasepassword == $password) {
            ...
            if ($conformpassword == $newpassword) {
                ...
                $this->session->set_flashdata('flux_errormsg', gettext("Password changed Sucessfully....!!!"));
                redirect(base_url() . 'user/user/changepassword/');
            } else {
                $this->session->set_flashdata('flux_notification', gettext("New Password & Conformpassword not match."));
                redirect(base_url() . 'user/user/changepassword/');
            }
        } else {
            $this->session->set_flashdata('flux_notification', gettext("Invalid old passwword."));
            redirect(base_url() . 'user/user/changepassword/');
        }
    }
...
function user_invoice_config()
{
    ...
    if (isset($add_array['submit'])) {
        ...
        if (isset($_FILES['file']['name']) && $_FILES['file']['name'] != '') {
            $files = $_FILES['file'];
            if ($files['size'] < 0) {
                $this->session->set_flashdata('flux_notification', gettext('PLease upload maximum file'));
            ...
            if ($file_type == 'image/jpg' || $file_type == 'image/png' || $file_type == 'image/jpeg') {
                ...
                $this->session->set_flashdata('flux_notification', gettext('Please upload only image!'));
                redirect(base_url() . 'user/user_invoice_config/');
            }
        }
    ...
    if ($add_array['id'] != '') {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($add_array['id'] == $accountinfo['id']) {
                ...
                $this->session->set_flashdata('flux_notification', gettext('Something wrong.Please contact to administrator.'));
            }
        }
    } else {
        $where = array(
...
function user_fund_transfer()
{
    ...
    if ($accountinfo['posttoexternal'] != '1') {
        ...
        $this->session->set_flashdata('flux_danger_alert', 'Permission Denied!');
        redirect(base_url() . 'user/user/');
    }
}
            ...
            if (trim($post_array['fromaccountid']) != trim($post_array['toaccountid'])) {
                ...
                if (! empty($toaccountinfo)) {
                    if ($toaccountinfo['posttoexternal'] == 1) {
                        $this->session->set_flashdata('flux_notification', gettext('Can not trnasfer fund to postpaid customer'));
                        ...
                        if ($post_array['toaccountid']) {
                            ...
                            if (($toaccountinfo['type'] != 3) && ($toaccountinfo['type'] != 0)) {
                                ...
                                $this->session->set_flashdata('flux_notification', gettext('Access Denied! unable transfer fund to this account'));
                            }
                            ...
                            if ($post_array['toaccountid'] == $account_info['number']) {
                                $this->session->set_flashdata('flux_notification', gettext('You can not transfer fund in same account.'));
                            } elseif ($reseller_id != $account_info['reseller_id']) {
                                $this->session->set_flashdata('flux_notification', gettext('You can only transfer fund in same level account.'));
                            } elseif ($post_array['toaccountid'] == '') {
                                $this->session->set_flashdata('flux_notification', gettext('Please enter To account number.'));
                            } elseif (empty($post_array['credit'])) {
                                $this->session->set_flashdata('flux_notification', gettext('Please enter valid amount.'));
                            } elseif ($post_array['credit'] > $balance) {
                                $this->session->set_flashdata('flux_notification', gettext('You have insufficient balance.'));
                            } elseif ($toid <= 0 || ! isset($post_array['toaccountid'])) {
                                $this->session->set_flashdata('flux_notification', gettext('Please enter valid account number.'));
                            } elseif ($post_array['credit'] < 0) {
                                $this->session->set_flashdata('flux_notification', gettext('Please enter amount greater then 0.'));
                            } elseif ($minimum_fund['value'] > $post_array['credit']) {
                                $this->session->set_flashdata('flux_notification', gettext(sprintf('You need to enter minimum %s for fund transfer. ', $minimum_fund['value'])));
                            } elseif (! isset($toid) || ! isset($post_array['toaccountid'])) {
                                $this->session->set_flashdata('flux_notification', gettext('Please enter valid account number!'));
                            } elseif ($post_array['credit'] < 0 || $post_array['credit'] > $balance) {
                                $this->session->set_flashdata('flux_notification', gettext('Insuffiecient amount !'));
                            } elseif (!$this->check_calls_running()) {
                                $this->session->set_flashdata('flux_notification', gettext('You can not transfer fund when your calls are running!'));
                            } else {
                                ...
                                $this->session->set_flashdata('flux_errormsg', gettext('Transfer successfully!'));
                            }
                        } else {
                            $this->session->set_flashdata('flux_notification', gettext('Account number not found.'));
                        }
                    }
                } else {
                    $this->session->set_flashdata('flux_notification', gettext('Account number not found.'));
...
function user_opensips_delete($id)
{
    ...
    $this->session->set_flashdata('flux_errormsg', gettext('Opensips Device Removed Successfully!.'));
    redirect(base_url() . "user/user_opensips/");
}
...
function user_speeddial_remove()
{
    ...
    if (empty($add_array['destination'])) {
        $this->session->set_flashdata('flux_notification', gettext('Can not delete blank speeddial number'));
        die();
    }
...
function user_pin_save()
{
    ...
    $this->session->set_flashdata('flux_errormsg', gettext('Pin Updated Successfully!.'));
...
function user_get_order_list_json()
{
    ...
    foreach ($query->result_array() as $key => $value) {
        $order_url = '<a href="' . base_url() . '/user/user_orders_complete/' . $value['order_id'] . '" style="color:#3b3280"  title="Edit" >' . $this->common->get_order_id("order_id", "orders", $value['id']) . '</a>';
        $json_data['rows'][] = array(
            'cell' => array(
                $value['order_date'],
                $order_url,
...
function user_fssipdevices_build_extension_dropdown(){
    $this->load->module ( "siprouting" );
    $this->siprouting->fssipdevices_build_extension_dropdown ();
}   

function user_fssipdevices_routing_save(){
    $this->load->module ( "siprouting" );
    $this->siprouting->fssipdevices_routing_save ();
}
