...
class Dashboard_model extends CI_Model
{
	...
	function get_customer_maximum_callminutes($start_date, $end_date, $parent_id, $scope_field = 'reseller_id')
	{
		$this->db->select('SUM(billseconds) AS billseconds, account_id', false);
		$this->db->from('cdrs_day_by_summary');
		$this->db->where($scope_field, $parent_id);
		$this->db->where('calldate >=', $start_date . ' 00:00:00');
		$this->db->where('calldate <=', $end_date . ' 23:59:59');
		$this->db->group_by('account_id');
		$this->db->order_by('billseconds', 'DESC');
		$this->db->limit(10);

		return $this->db->get();
	}

	function get_customer_maximum_callcount($start_date, $end_date, $parent_id, $scope_field = 'reseller_id')
	{
		$this->db->select('SUM(total_calls) AS call_count, account_id', false);
		$this->db->from('cdrs_day_by_summary');
		$this->db->where($scope_field, $parent_id);
		$this->db->where('calldate >=', $start_date . ' 00:00:00');
		$this->db->where('calldate <=', $end_date . ' 23:59:59');
	...
	function get_customer_maximum_countryminutes($start_date, $end_date, $parent_id, $scope_field = 'reseller_id')
	{
		$sql = "SELECT SUM(cdrs.billseconds) AS billseconds, calltype.id AS call_type_id
		        FROM cdrs
		        JOIN calltype ON cdrs.calltype = calltype.call_type
		        WHERE cdrs.{$scope_field} = ?
		          AND cdrs.callstart >= ?
		          AND cdrs.callstart <= ?
		        GROUP BY calltype.call_type, calltype.id
		        ORDER BY billseconds DESC
		        LIMIT 10";
	...
	function get_customer_maximum_countrycount($start_date, $end_date, $parent_id, $scope_field = 'reseller_id')
	{
		$sql = "SELECT COUNT(cdrs.calltype) AS call_count, calltype.call_type, calltype.id AS call_type_id
		        FROM cdrs
		        JOIN calltype ON cdrs.calltype = calltype.call_type
		        WHERE cdrs.{$scope_field} = ?
		          AND cdrs.callstart >= ?
