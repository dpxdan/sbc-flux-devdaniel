...
function process_cdr($data, $db, $logger, $decimal_points, $config) {
	...
	$dataVariable ['caller_id_type'] = isset ( $dataVariable ['caller_id_type'] ) ? $dataVariable ['caller_id_type'] : "none";
	
	$dataVariable ['check_carrier_type'] = isset ( $dataVariable ['check_carrier_type'] ) ? $dataVariable ['check_carrier_type'] : "none";
	
	$dataVariable ['check_carrier'] = isset ( $dataVariable ['check_carrier'] ) ? $dataVariable ['check_carrier'] : "none";


	if($dataVariable ['check_carrier'] == 'caller_id'){	
	$dataVariable ['caller_carrier_id'] = $dataVariable ['caller_carrier_id'];
	$dataVariable ['caller_call_id_cadup'] = $dataVariable ['caller_idCadup'];
	$dataVariable ['carrier_id'] = 0;
	$dataVariable ['idCadup'] = 0;
	$dataVariable ['carrier_type'] = 'caller_id';	
	}
	elseif($dataVariable ['check_carrier'] == 'destination_number'){	
	$dataVariable ['caller_carrier_id'] = 0;
	$dataVariable ['caller_call_id_cadup'] = 0;
	$dataVariable ['carrier_id'] = $dataVariable ['carrier_id'];
	$dataVariable ['idCadup'] = $dataVariable ['idCadup'];
	$dataVariable ['carrier_type'] = 'destination_number';
	}
	elseif($dataVariable ['check_carrier'] == 'both'){	
	$dataVariable ['caller_carrier_id'] = $dataVariable ['caller_carrier_id'];
	$dataVariable ['caller_call_id_cadup'] = $dataVariable ['caller_idCadup'];
	$dataVariable ['carrier_id'] = $dataVariable ['carrier_id'];
	$dataVariable ['idCadup'] = $dataVariable ['idCadup'];
	$dataVariable ['carrier_type'] = 'both';
	}
	else {
	$dataVariable ['caller_carrier_id'] = 0;
	$dataVariable ['caller_call_id_cadup'] = 0;	
	$dataVariable ['carrier_id'] = 0;
	$dataVariable ['idCadup'] = 0;
	$dataVariable ['carrier_type'] = 'none';
	}
	
	if($dataVariable ['caller_id_type'] != 'none'){
	$dataVariable ['effective_caller_id_name'] = $dataVariable ['effective_caller_id_name'];
	$dataVariable ['effective_caller_id_number'] = $dataVariable ['effective_caller_id_number'];
	$dataVariable ['original_caller_id_name'] = $dataVariable ['original_caller_id_name'];
	$dataVariable ['original_caller_id_number'] = $dataVariable ['original_caller_id_number'];
	}
	
	
	$logger->log("checkCadup:::".$checkCadup);
	$logger->log("idCadup:::".$idCadup);
	$logger->log("carrier_id:::".$carrier_id);
	$logger->log("nomeLocalidade:::".$nomeLocalidade);
	$logger->log("nomePrestadora:::".$nomePrestadora);
	$logger->log("areaLocal:::".$areaLocal);
	$logger->log("tipo:::".$tipo);
	$logger->log("prefixo:::".$prefixo);
	$logger->log("codArea:::".$codArea);
	$logger->log("uf:::".$uf);
	$logger->log("caduprn1:::".$rn1);
	$logger->log("force_trunk_flag:::".$force_trunk_flag);
	$logger->log("rate_flag:::".$rate_flag);
	$logger->log("decimal_points:::".$decimal_points);
	...
	//$logger->log(print_r($origination_rate,true));
		...
		if($dataVariable['routing_type'] == 4){
            ...
            $logger->log("*********************** Harsh_routing_type in routing_type: **".$routing_type."***********");
            ...
            $logger->log("*********************** termination_rate_group in routing_type: **".$termination_rate_group."***********");
            $query = "update carrier_routing SET call_count = call_count+1 where id =".$carrier_route_id;
            $logger->log("GET Update pricelist count decrease  : " . $query);
	...
	if($actual_duration > 0 || $dataVariable['hangup_cause'] == 'NORMAL_CLEARING' || $dataVariable['hangup_cause'] == 'SUCCESS' || $dataVariable['hangup_cause']  == 'ALLOTTED_TIMEOUT' || $dataVariable['hangup_cause'] == 'USER_BUSY' || $dataVariable['hangup_cause']  == 'INTERWORKING'){
		...
		if($dataVariable['force_trunk_flag'] > 0 ){
		    ...
		    $logger->log("GET Update route_query count decrease  : " . $route_query);
			...
			if($dataVariable['rate_flag'] == 4){
				...
				$logger->log("*********************** Harsh_rategroup in CDRs: **".$carrier_rate_id."***********");
		...
		$carrier_rate_id= $termination_rate['ID'];
		$logger->log("*********************** Harsh_trunk in CDRs: **".$carrier_rate_id."***********");
		$logger->log("*********************** trunk_id in CDRs: **".$trunk_id."***********");
		if($dataVariable['force_trunk_flag'] > 0){
			$trunk_query = "update routing SET call_count = call_count+1 where trunk_id=".$trunk_id;
			$logger->log("GET Update trunk_query165 count decrease  : " . $trunk_query);
		}
		else{
			 if($dataVariable['rate_flag'] == 3){
				...
				$logger->log("GET Update trunk_query169 count decrease  : " . $trunk_query);
			 }
			 else{
				$trunk_query = "update routing SET call_count = call_count-1 where trunk_id=".$trunk_id." and pricelist_id =".$rate_group;
		}
		}
		$logger->log("GET Update routing count decrease  : " . $trunk_query);
	...
	$logger->log ( "*********************** ADDING EXTRA ENTRY For local/DID Inbound call *************" );
	$receiver_parentid = 0;
	if (isset ( $dataVariable ['call_type_custom'] ) && $dataVariable ['call_type_custom'] != "DID-REVERSE"){
...
function process_file_cdr($data, $db, $logger, $decimal_points, $config) {
	...
	if ($dataVariable ['callstart'] == ""){
	return 'no_callstart';
	}
	
	$logger->log("dataVariables::" . json_encode($dataVariable));
	$logger->log("dataCallflow::" . json_encode($dataCallflow));
 ...
 */
function insert_parent_data($dataVariable, $actual_calltype, $parentid, $origination_rate, $actual_duration, $provider_cost, $flag_parent, $logger, $db, $decimal_points, $config, $trunk_id='') {
	while ( $parentid > 0 ) {
		...
		$block_duration = calc_block_duration( $dataVariable, $origination_rate [$accountid], $logger);
		$logger->log ( "*************** RESELLER BLOCK DURATION********" );
		// If receiver account id found then explicitly set call direction and call type
		/*
		 * if(isset($dataVariable['receiver_accid']))
		 * {
		...
		if (isset ( $dataVariable ['receiver_accid'] ) && $dataVariable ['receiver_accid'] != "" && $flag_parent == true) {
			...
			$flag_parent = true;
			insert_extra_receiver_entry ( $dataVariable, $origination_rate, $termination_rate, $account_type, $actual_duration, $provider_cost, $parentid, $flag_parent, $accountid, $logger, $db, $decimal_points, $config ,$trunk_id, $block_duration);
			
			$logger->log ( "********* IN RESELLER FOR RECEIVER ENTRY END ******" );
		} else {
			$logger->log ( "********* RESELLER CDR ******" );
			$cdr_string = get_reseller_cdr_string ( $dataVariable, $accountid, $account_type, $actual_duration, $termination_rate, $origination_rate, $provider_cost, $parentid, $debit, $cost,$logger,$block_duration,$db,$trunk_id);
			$logger->log ( "********* RESELLER CDR STRING: " . $cdr_string);
			$query = "INSERT INTO reseller_cdrs (uniqueid,accountid,callerid,callednum,billseconds,disposition,callstart,debit,cost,pricelist_id,package_id,pattern,notes,rate_cost,reseller_id,reseller_code,reseller_code_destination,reseller_cost,call_direction,calltype,call_request,country_id,end_stamp,trunk_id,block_billseconds) values ($cdr_string)";
			$logger->log ( $query );
			$db->run ( $query );
 ...
 * @param boolean $flag_parent        	
 */
function insert_extra_receiver_entry($dataVariable, $origination_rate, $termination_rate, $account_type, $actual_duration, $provider_cost, $parentid, $flag_parent, $accountid, $logger, $db, $decimal_points, $config,$trunk_id='', $block_duration) {
	...
	if ($dataVariable ['calltype'] == "Local") {
		...
		if ($flag_parent == false) {
			$cdr_string = get_cdr_string ( $localVariable, $accountid, $account_type, $actual_duration, $termination_rate, $origination_rate, $provider_cost, $parentid, 0, 0, $block_duration, $logger, $db);
		} else {
			$cdr_string = get_reseller_cdr_string ( $dataVariable, $accountid, $account_type, $actual_duration, $termination_rate, $origination_rate, $provider_cost, $parentid, $debit, $cost,$logger,$block_duration,$db ,$trunk_id);
		}
	} else {
		...
		if ($flag_parent == false) {
			...
			$cdr_string = get_cdr_string ( $localVariable, $accountid, $account_type, $actual_duration, $termination_rate, $origination_rate_did, $provider_cost, $parentid, $debit, 0, $block_duration, $logger, $db );
		} else {
			$cdr_string = get_reseller_cdr_string ( $dataVariable, $accountid, $account_type, $actual_duration, $termination_rate, $origination_rate, $provider_cost, $parentid, $debit, $cost, $block_duration,$logger,$db);
		}
	}
//	$dataVariable ['sip_user'] = '';
	if ($flag_parent == false) {
		...
		$query = "INSERT INTO reseller_cdrs (uniqueid,accountid,callerid,callednum,billseconds,disposition,callstart,debit,cost,pricelist_id,package_id,pattern,notes,rate_cost,reseller_id,reseller_code,reseller_code_destination,reseller_cost,call_direction,calltype,call_request,country_id,end_stamp,block_billseconds) values ($cdr_string)";
	}
	
	$logger->log ( $query );
	$db->run ( $query );
	
	//if ($debit > 0 && ($dataVariable ['calltype'] != "Gratuita" && $dataVariable ['calltype'] != "LOCAL")) {
	if ($debit > 0 && ($dataVariable ['calltype'] != "Gratuita" && $dataVariable ['calltype'] != "LOCAL")) {
		update_balance ( $accountid, $debit, 0, $logger, $db, $config,$dataVariable );
	}
	return true;
...
function get_cdr_string($dataVariable, $accountid, $account_type, $actual_duration, $termination_rate, $origination_rate, $provider_cost, $parentid, $debit, $cost, $block_duration, $logger, $db) {
...
function calc_block_duration($dataVariable, $rates, $logger) {
	// $logger->log(print_r($rates,true));
	$duration = (int) isset($dataVariable['billsec'])?$dataVariable ['billsec']:0;
	$block_duration = 0;
	...
	if ($duration > 0) {
		...
		$rates ['INITIALBLOCK'] = (empty($rates['INITIALBLOCK']) || $rates['INITIALBLOCK'] < 1) ? 30 : $rates ['INITIALBLOCK']; //HP: change default value 1 to 0 when 'INITIALBLOCK' emotry or lessthen 1.
		$duration -= $rates ['INITIALBLOCK'];
		$block_duration += $rates ['INITIALBLOCK'];
		
		if ($duration > 0) {

			$rates ['INC'] = (empty($rates['INC']) || $rates['INC'] < 1) ? 1 : $rates ['INC'];

			$blocks = ceil($duration / $rates['INC']);
			$block_duration += $blocks * $rates['INC'];
		}
	}
	$logger->log ( "INITIALBLOCK " . $rates ['INITIALBLOCK'] );
	$logger->log ( "INC " . $rates ['INC'] );
	$logger->log ( "COST " . $rates ['COST'] );
	$logger->log ( "BLOCK DURATION " . $block_duration);
